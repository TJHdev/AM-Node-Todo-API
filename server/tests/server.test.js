...
describe('GET /todos', ()=>{
    it('should return one todo', (done) => {
        request(app)
            ...
            .expect(200)
            .expect((res)=>{
                expect(res.body.todos.length).toBe(1) // do not need to use the .end call as we're not doing anything asyncronously
            })
            .end(done);
    });
});
...
describe('POST /users', () => {
    it('should create a user', (done) => {
        let email = 'tomexample@gmail.com'
        let password = '123mnb!';
    ...
    it('should return validation errors if request invalid', (done) => {
        //make a request with a password that is too short
        //make a request with an invalid email (no @)
        request(app)
            .post('/users')
            .send({
            ...
            .expect(400)
            .expect((res) => {
                expect(res.body).toHaveProperty('_message', 'User validation failed')
            })
            .end(done);
    });

    it('should not create user if email in use', (done) => {
        //make a request with an email that is in use
        request(app)
            .post('/users')
            .send({
                email: users[0].email, 
                password:'newUserOnePass'
            })
            .expect(400)
            .expect((res) => {
                expect(res.body).toHaveProperty('code', 11000)
            })
            .end(done);
    });
});
    ...
    it('should login user and return auth token', (done) => {
        request(app)
            ...
            .expect((res) => {
                ...
                // expect(res.body._id).toBe(users[1]._id.toString())
                expect(res.headers['x-auth']).toBeTruthy();
            })
            .end((err, res) => {
                if (err) {
